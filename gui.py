...
from controller_sim import (DifferentialEqnThread, RealSystemThread, PINNModelThread, 
                            TransferFunctionModelThread, DataDrivenModelThread
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.select_model.addItem("Transfer Function Model")
        self.select_model.addItem("Data Driven Model")
        ...
        self.checkbox_transfer_function = QCheckBox("Tf Model Plot")
        self.checkbox_transfer_function.setChecked(True)
        self.sidebar_layout.addWidget(self.checkbox_transfer_function)

        self.checkbox_data_driven = QCheckBox("Data Driven Model Plot")
        self.checkbox_data_driven.setChecked(True)
        self.sidebar_layout.addWidget(self.checkbox_data_driven)
        ...
        # Block Diagram
        self.image_path = "./images/block_diagram_OL.png"  # Replace with your image path
        self.text_data_OL = {
            "Real System_height": (f"y: {0}", (600, 280), "orange"),
            "Differential_height": (f"y: {0}", (600, 320), "blue"),
            "PINN_height": (f"y: {0}", (600, 340), "green"),
            "Transfer Function_height": (f"y: {0}", (600, 360), "red"),
            "Data Driven_height": (f"y: {0}", (600, 380), "purple"),
            "voltage": (f"u: {self.set_point_height}", (180, 300), "gray"), 
            "Real System_flow_rate": ("Qin: 0", (360, 280), "orange"),
            "Differential_flow_rate": ("Qin: 0", (360, 320), "blue"),
            "PINN_flow_rate": ("Qin: 0", (360, 340), "green"),
            "Transfer Function_flow_rate": ("Qin: 0", (360, 360), "red"),
            "Data Driven_flow_rate": ("Qin: 0", (360, 380), "purple"),
        }

        self.text_data_CL = {
            "kp": (f"Kp: {self.kp}", (325, 225), "gray"),
            "ki": (f"Ki: {self.ki}", (325, 255), "gray"),
            "kd": (f"Kd: {self.kd}", (325, 285), "gray"),
            "set_point": (f"SP: {self.set_point_height}", (100, 220), "gray"),
            "Real System_error": ("Error: 0", (240, 200), "orange"),
            "Differential_error": ("Error: 0", (240, 240), "blue"),
            "PINN_error": ("Error: 0", (240, 260), "green"),
            "Transfer Function_error": ("Error: 0", (240, 280), "red"), 
            "Data Driven_error": ("Error: 0", (240, 300), "purple"),
            "Real System_height": ("y: 2.0", (665, 175), "orange"),
            "Differential_height": ("y: 2.0", (665, 215), "blue"),
            "PINN_height": ("y: 2.0", (665, 235), "green"),
            "Transfer Function_height": ("y: 2.0", (665, 255), "red"),
            "Data Driven_height": ("y: 2.0", (665, 275), "purple"),
            "voltage": ("u: 0.0", (380, 205), "gray"),
            "Real System_flow_rate": ("Qin: 13.5", (485, 175), "orange"),
            "Differential_flow_rate": ("Qin: 13.5", (485, 215), "blue"),
            "PINN_flow_rate": ("Qin: 13.5", (485, 235), "green"), 
            "Transfer Function_flow_rate": ("Qin: 13.5", (485, 255), "red"),
            "Data Driven_flow_rate": ("Qin: 13.5", (485, 275), "purple")
        }
        ...
        self.transfer_function_thread = TransferFunctionModelThread(set_point_height=0.0, kp=30.0, ki=1.0, kd=0.0, open_loop=True)
        self.data_driven_thread = DataDrivenModelThread(set_point_height=0.0, kp=30.0, ki=1.0, kd=0.0, open_loop=True)
        ...
        self.transfer_function_thread.update_height.connect(lambda v, h, t: self.update_plot(v, h, t, "Transfer Function"))
        self.data_driven_thread.update_height.connect(lambda v, h, t: self.update_plot(v, h, t, "Data Driven"))
        ...
        self.transfer_function_thread.error_signal.connect(self.show_error_message)
        self.data_driven_thread.error_signal.connect(self.show_error_message)
    ...
    def confirm_model(self):
        keys = ["Differential_data", "Real System_data", "PINN_data", "Transfer Function_data", "Data Driven_data"]
        curr_date = date.today()
        if self.select_model.currentText() == "Differential Model":
            path = f"./data/{keys[0]}_OL_{curr_date}_{self.save_time}.csv"
        elif self.select_model.currentText() == "Real System Model":
            path = f"./data/{keys[1]}_OL_{curr_date}_{self.save_time}.csv"
        elif self.select_model.currentText() == "PINN Model":
            path = f"./data/{keys[2]}_OL_{curr_date}_{self.save_time}.csv"
        elif self.select_model.currentText() == "Transfer Function Model":
            path = f"./data/{keys[3]}_OL_{curr_date}_{self.save_time}.csv"
        elif self.select_model.currentText() == "Data Driven Model":
            path = f"./data/{keys[4]}_OL_{curr_date}_{self.save_time}.csv"
    ...
    def update_close_loop(self):
        ...
        kc, ki, kd = self.pid_calculator.kc, self.pid_calculator.ki, self.pid_calculator.kd
        self.text_data_CL["set_point"] = (f"SP: {self.setpoint_input.text()}", (115, 225), "gray")
        self.text_data_CL["kp"] = (f"Kp: {kc}", (310, 205), "gray")
        self.text_data_CL["ki"] = (f"Ki: {ki}", (310, 235), "gray")
        self.text_data_CL["kd"] = (f"Kd: {kd}", (310, 265), "gray")
    ...
    def update_parameters(self):
        ...
        self.transfer_function_thread.set_point_height = set_point_height
        self.data_driven_thread.set_point_height = set_point_height
        self.text_data_OL["voltage"] = (f"u: {set_point_height}", (210, 305), "gray")
        self.text_data_CL["set_point"] = (f"SP: {set_point_height}", (115, 225), "gray")
        ...
        self.transfer_function_thread.pid.setpoint = set_point_height
        self.data_driven_thread.pid.setpoint = set_point_height
    ...
    def start_threads(self):
        ...
        self.transfer_function_thread.start()
        self.data_driven_thread.start()
    ...
    def stop_threads(self):
        ...
        self.transfer_function_thread.terminate()
        self.data_driven_thread.terminate()
        self.diff_eqn_thread.wait()
        self.real_system_thread.wait()
        self.pinn_thread.wait()
        self.transfer_function_thread.wait()
        self.data_driven_thread.wait()
        ...
        for key in keys:
            try:
                curr_date = date.today()
                os.rename(f"./data/{key}.csv", f"./data/{key}_{self.curr_loop}_{curr_date}_{self.save_time}.csv")
    ...
    def update_plot(self, voltage, height, time_str, model_type):
        ...
        height_data = {"Differential": [], "Real System": [], "PINN": [], "Transfer Function": [], "Data Driven": []}
        voltage_data = {"Differential": [], "Real System": [], "PINN": [], "Transfer Function": [], "Data Driven": []}
        time_data = {"Differential": [], "Real System": [], "PINN": [], "Transfer Function": [], "Data Driven": []}

        # Save data to CSV
        self.save_data_to_csv(model_type)

        # if len(self.height_data[model_type]) > 1000:
        if False:
            # Retain the last 10 points for all models
            for model in self.height_data.keys():
                height_data[model] = self.height_data[model][-1000:]
                voltage_data[model] = self.voltage_data[model][-1000:]
                time_data[model] = self.time_data[model][-1000:]
        else:
            # Use all available data for all models
            for model in self.height_data.keys():
                height_data[model] = self.height_data[model]
                voltage_data[model] = self.voltage_data[model]
                time_data[model] = self.time_data[model]

        # Update Graph 1 (Height vs Time)
        self.ax1.clear()
        model = list(self.height_data.keys())
        if self.checkbox_diff_eqn.isChecked():
            self.ax1.plot(time_data[model[0]], height_data[model[0]], label=f"{model[0]} Model", color="blue")
        if self.checkbox_real_system.isChecked():
            self.ax1.plot(time_data[model[1]], height_data[model[1]], label=f"{model[1]} Model", color="orange")
        if self.checkbox_pinn.isChecked():
            self.ax1.plot(time_data[model[2]], height_data[model[2]], label=f"{model[2]} Model", color="green")
        if self.checkbox_transfer_function.isChecked():
        ...
        # Configure X-axis
        self.configure_x_axis(self.ax1, time_data)
        ...
        self.ax2.clear()
        if self.checkbox_diff_eqn.isChecked():
            self.ax2.plot(time_data[model[0]], voltage_data[model[0]], label=f"{model[0]} Model", color="blue")
        if self.checkbox_real_system.isChecked():
            self.ax2.plot(time_data[model[1]], voltage_data[model[1]], label=f"{model[1]} Model", color="orange")
        if self.checkbox_pinn.isChecked():
            self.ax2.plot(time_data[model[2]], voltage_data[model[2]], label=f"{model[2]} Model", color="green")
        if self.checkbox_transfer_function.isChecked():
            self.ax2.plot(time_data[model[3]], voltage_data[model[3]], label=f"{model[3]} Model", color="red")
        if self.checkbox_data_driven.isChecked():
            self.ax2.plot(time_data[model[4]], voltage_data[model[4]], label=f"{model[4]} Model", color="purple")

        try:
            set_point = float(self.setpoint_input.text())
        except Exception as e:
            pass
        ...
        if self.curr_loop == "CL":
            if model_type == "Differential":
                self.text_data_CL[f"{model_type}_height"] = (f"y: {round(height,3)}", (665, 215), "blue")
                self.text_data_CL[f"{model_type}_error"] = (f"Error: {round(float(self.setpoint_input.text())-height,3)}", (240, 240), "blue")
                self.text_data_CL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (485, 215), "blue")
            elif model_type == "Real System":
                self.text_data_CL[f"{model_type}_height"] = (f"y: {round(height,3)}", (665, 175), "orange") 
                self.text_data_CL[f"{model_type}_error"] = (f"Error: {round(float(self.setpoint_input.text())-height,3)}", (240, 200), "orange")
                self.text_data_CL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (485, 175), "orange")
            elif model_type == "PINN":
                self.text_data_CL[f"{model_type}_height"] = (f"y: {round(height,3)}", (665, 235), "green")
                self.text_data_CL[f"{model_type}_error"] = (f"Error: {round(float(self.setpoint_input.text())-height,3)}", (240, 260), "green")
                self.text_data_CL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (485, 235), "green")
            elif model_type == "Transfer Function":
                self.text_data_CL[f"{model_type}_height"] = (f"y: {round(height,3)}", (665, 255), "red")
                self.text_data_CL[f"{model_type}_error"] = (f"Error: {round(float(self.setpoint_input.text())-height,3)}", (240, 280), "red")
                self.text_data_CL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (485, 255), "red")
            elif model_type == "Data Driven":
                self.text_data_CL[f"{model_type}_height"] = (f"y: {round(height,3)}", (665, 275), "purple")
                self.text_data_CL[f"{model_type}_error"] = (f"Error: {round(float(self.setpoint_input.text())-height,3)}", (240, 300), "purple")
                self.text_data_CL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (485, 275), "purple")
            self.text_data_CL["voltage"] = (f"u: {round(voltage,3)}", (380, 205), "gray")
            self.block_diagram.params(self.text_data_CL)
        else:
            if model_type == "Differential":
                self.text_data_OL[f"{model_type}_height"] = (f"y: {round(height,3)}", (600, 320), "blue")
                self.text_data_OL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (360, 320), "blue")
            elif model_type == "Real System":
                self.text_data_OL[f"{model_type}_height"] = (f"y: {round(height,3)}", (600, 280), "orange")
                self.text_data_OL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (360, 280), "orange")
            elif model_type == "PINN":
                self.text_data_OL[f"{model_type}_height"] = (f"y: {round(height,3)}", (600, 340), "green")
                self.text_data_OL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (360, 340), "green")
            elif model_type == "Transfer Function":
                self.text_data_OL[f"{model_type}_height"] = (f"y: {round(height,3)}", (600, 360), "red")
                self.text_data_OL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (360, 360), "red")
            elif model_type == "Data Driven":
                self.text_data_OL[f"{model_type}_height"] = (f"y: {round(height,3)}", (600, 380), "purple")
                self.text_data_OL[f"{model_type}_flow_rate"] = (f"Qin: {round(height*10,3)}", (360, 380), "purple")

            self.text_data_OL["voltage"] = (f"u: {self.setpoint_input.text()}", (210, 305), "gray")
            self.block_diagram.params(self.text_data_OL)
        ...
        # Configure X-axis
        self.configure_x_axis(self.ax2, time_data)
    ...
    def epoch_to_hhmmss(self, epoch, pos=None):
    ...
    def configure_x_axis(self, ax, time_data):
        ...
        formatter = FuncFormatter(self.epoch_to_hhmmss)
        ...
        for model, time_values in time_data.items():
            ...
            ax.set_xticks(displayed_ticks)  # Set raw epoch values as ticks
            ax.xaxis.set_major_formatter(formatter)  # Use formatter for labels
        
        # Automatically handles rotation and fontsize
        ax.tick_params(axis='x', rotation=45, labelsize=8)


    def save_data_to_csv(self, model_type):
        """ Save data to separate CSV files for each model. """
        with open(f"./data/{model_type}_data.csv", mode="a", newline='') as file:
